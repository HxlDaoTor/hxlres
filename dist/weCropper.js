...
function cut() {
	...
	self.setBoundStyle = function (ref) {
		...
		self.ctx.fillRect(0, 0, x, boundHeight);  // 左
		self.ctx.fillRect(x, 0, width, y);                    //上
    self.ctx.fillRect(x, y + height, width, boundHeight - y - height);          //下
		self.ctx.fillRect(x + width, 0, boundWidth - x - width, boundHeight);              //右
		self.ctx.fill();

  // 画布上的提示文本
    self.ctx.setFontSize(12)
    self.ctx.setFillStyle('#FFFFFF')
    self.ctx.setTextAlign('left')
    self.ctx.fillText('移动、缩放图片进行裁剪', 120, 20)
    self.ctx.fillText('☆请勿上传违规违禁图片', 120, y + height + 30)
    self.ctx.stroke();
    ...
    self.ctx.setLineWidth(1);
    self.ctx.moveTo(x-1, y-1); //起点
    self.ctx.lineTo(x-1, y +1 + width/2);               //左
    self.ctx.lineTo(x+ width+1, y+1  + width/2);               //下
    self.ctx.lineTo(x+ width+1, y-1);             //右
    self.ctx.lineTo(x-1.5, y-1);       //上
    self.ctx.stroke();
	};
}
